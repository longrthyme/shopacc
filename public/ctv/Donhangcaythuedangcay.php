...
if(isset($_POST['Save']) && $getUser['ctv'] == 1 )
{
    ...
    $row = $NDVMEDIA->get_row("SELECT * FROM `orders_caythue` WHERE `id` = '$id' ");
    if(!$row)
    {
    ...
    $NDVMEDIA->update("orders_caythue", array(
        'status' => $status,
        'reason' => $reason,
...
if(isset($_POST['btnTimKiem']))
{
    ...
    $list = $NDVMEDIA->get_list(" SELECT * FROM `orders_caythue` WHERE  `status` = 'xuly' OR `receiver` = '".$getUser['username']."' AND `createdate` >= '".$daterangepicker[0]."' AND `createdate` <= '".$daterangepicker[1]."' ORDER BY id DESC ");
}  
else
{

    $list = $NDVMEDIA->get_list(" SELECT * FROM `orders_caythue` WHERE `status` = 'xuly' OR `receiver` = '".$getUser['username']."' ORDER BY id DESC ");
        ...
        <div class="content-wrapper">
          <div class="row">
           <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">CÀY THUÊ  XỬ LÍ</h4>
                    ...
                    NDV MEDIA
                            ...
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `orders_caythue` WHERE `status` = 'dangcay' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['username'];?></td>
