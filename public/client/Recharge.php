...
<div class="lq-content uk-position-relative" style="margin-top: 20px !important;">
    <div class="uk-container" id="category">
        ...
        <?php if($NDVMEDIA->site('luuy_naptien') != NULL) { ?>
        <div class="alert alert-info">
            <?=$NDVMEDIA->site('luuy_naptien');?>
        </div>
        <?php }?>
                ...
                <div class="table-responsive">
                    <table id="datatable" style="color: white;" class="table table-striped table-hover text-center">
                        ...
                        <tbody>
                            <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `cards` WHERE `username` = '".$getUser['username']."'  ORDER BY id DESC ") as $row){
                                    ?>
                            <tr>
                                <td><?=$i++;?></td>
                                <td><?=$row['seri'];?></td>
