...
if(isset($_GET['delete']) && $getUser['level'] == 'admin')
{
    ...
    $isDelete = $NDVMEDIA->remove("groups_robux", " `id` = '$delete' ");
    if($isDelete)
    {
        admin_msg_success("Xóa thành công !", BASE_URL('public/admin/GroupsRobux.php?id='.$_GET['id']), 500);
    }
}
...
if(isset($_POST['ThemChuyenMuc']) && $getUser['level'] == 'admin' )
{
    ...
    $NDVMEDIA->insert("groups_robux", array(
                  ...
                  <p class="card-description">
                    ...
                    <div class="card-body">
                        <form action="" method="POST" enctype="multipart/form-data">
                            ...
                            <a type="button" href="<?=BASE_URL('public/admin/CategoryRobux.php');?>"
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `groups_robux` WHERE `category` = '".check_string($_GET['id'])."'  ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['title'];?></td>
                                        ...
                                        <td>
                                            <a class="btn btn-primary"
                                                href="<?=BASE_URL('public/admin/EditGroupsRobux.php?id='.$row['id']);?>"><i
                                                    class="fas fa-edit"></i>
                                                <span>EDIT</span></a>
                                            <a class="btn btn-danger"
                                                href="<?=BASE_URL('public/admin/GroupsRobux.php?delete='.$row['id'].'&id='.$_GET['id']);?>"><i
