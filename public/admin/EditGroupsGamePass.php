<?php
    ...
    $title = 'CHỈNH SỬA NHÓM GAMEPASS | '.$NDVMEDIA->site('tenweb');
...
{
    $row = $NDVMEDIA->get_row(" SELECT * FROM `groups_gamepass` WHERE `id` = '".check_string($_GET['id'])."'  ");
    if(!$row)
    {
...
if(isset($_POST['Save']) && $getUser['level'] == 'admin' )
{
    ...
    $NDVMEDIA->update("groups_gamepass", array(
