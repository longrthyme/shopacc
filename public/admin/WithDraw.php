...
if(isset($_POST['Save']) && $getUser['level'] == 'admin' )
{
    ...
    $row = $NDVMEDIA->get_row("SELECT * FROM `orders_withdraw` WHERE `id` = '$id' ");
    if(!$row)
    {
    ...
    {
        // refund
        $isrobux = $NDVMEDIA->cong("users", "robux", $row['robux'], " `username` = '".$row['username']."' ");
        if($isrobux)
        {
    ...
    $NDVMEDIA->update("orders_withdraw", array(
        'status' => $status,
        ...
        <div class="content-wrapper">
          <div class="row">
           <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">ĐƠN HÀNG RÚT ROBUX</h4>
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `orders_withdraw` ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['dichvu'];?></td>
