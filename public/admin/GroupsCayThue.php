...
if(isset($_GET['delete']) && $getUser['level'] == 'admin')
{
    ...
    $isDelete = $NDVMEDIA->remove("groups_caythue", " `id` = '$delete' ");
    if($isDelete)
    {
...
if(isset($_POST['ThemChuyenMuc']) && $getUser['level'] == 'admin' )
{
    ...
    $NDVMEDIA->insert("groups_caythue", array(
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `groups_caythue` WHERE `category` = '".check_string($_GET['id'])."'  ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['title'];?></td>
