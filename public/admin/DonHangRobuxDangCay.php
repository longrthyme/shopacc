...
if(isset($_POST['Save']) && $getUser['level'] == 'admin' )
{
    ...
    $row = $NDVMEDIA->get_row("SELECT * FROM `orders_robux` WHERE `id` = '$id' ");
    if(!$row)
    {
    ...
    $NDVMEDIA->update("orders_robux", array(
        'status' => $status,
        'reason' => $reason
    ), " `id` = '".check_string($_POST['id'])."' ");
        ...
        <div class="content-wrapper">
          <div class="row">
           <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">ROBUX ĐANG CÀY</h4>
                  <p class="card-description">
                            ...
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `orders_robux` WHERE `status` = 'dangcay' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['receiver'];?></td>
