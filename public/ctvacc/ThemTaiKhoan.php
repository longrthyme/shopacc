<?php
    ...
    $title = 'THÊM TÀI KHOẢN | '.$NDVMEDIA->site('tenweb');
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php foreach($NDVMEDIA->get_list("SELECT * FROM `groups` WHERE `display` = 'SHOW' ") as $row) { ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td width="10%"><img width="100%" src="<?=BASE_URL($row['img']);?>" /></td>
                                        <td><?=$row['title'];?></td>
                                        <td><?=format_cash($NDVMEDIA->num_rows(" SELECT * FROM `accounts` WHERE `groups` = '".$row['id']."' AND `receiver` = '".$getUser['username']."' "));?>
                                        </td>
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `accounts` WHERE `groups` = '".check_string($_GET['group'])."' AND `receiver` = '".$getUser['username']."'  ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-primary"
                                                href="<?=BASE_URL('public/ctvacc/EditAccount.php?id='.$row['id']);?>"><i
                                                    class="fas fa-edit"></i>
                                                <span>EDIT</span></a>
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NDVMEDIA->get_list(" SELECT * FROM `groups` WHERE `display` = 'SHOW'  ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td width="10%"><img width="100%" src="<?=BASE_URL($row['img']);?>" /></td>
                                        <td><?=$row['title'];?></td>
                                        <td><?=number_format($NDVMEDIA->num_rows("SELECT * FROM `groups` WHERE `category` = '".$row['id']."' "));?>
                                        </td>
                                        <td><?=display($row['display']);?></td>
                                        <td>
                                            <a type="button"
